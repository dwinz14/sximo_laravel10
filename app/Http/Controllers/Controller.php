	...
	function getComboselect( Request $request)
	{
		...
		if($request->ajax() == true && \Auth::check() == true)
		{
			...
			foreach($rows as $row) 
			{
				...
				$items[] = array($row->{$param['1']} , rtrim($value, " -> ")); 	
...
function detailviewsave( $model ,$request , $detail , $id )
	...
	public function getOneRow(Request $request)
	{
		if($request->ajax() == true && \Auth::check() == true)
		{
			...
			$data = \DB::table($table)->where($key,$value);
	
			if($data->count() > 0){
				return response()->json(array("status"=>"success", 'data'=> $data->first()));
			}else{
