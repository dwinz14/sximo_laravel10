...
class UserController extends Controller {
        ...
        if ($validator->passes()) {
            ...
            if (\Auth::attempt(array('email'=> $email, 'password'=> $password ))) {
                ...
                if($row->status =='0')
                {
					...
					$expired_token = new \DateTime();
					$expired_token->format("Y-m-d H:i:s");
					$expired_token->modify('+8 day');
                    \DB::table('tb_token')->updateOrInsert([
                    	'userId'	=> $row->id ,
                    	'token'		=> $token ,
                    	'created'	=> date("Y-m-d H:i:s"),
						'expired'   => $expired_token->format("Y-m-d H:i:s")
                	],['userId' => $row->id]);
	...
	function TokenValidation(Request $request)
	{
		...
		$dt_token = DB::table('tb_token')->where('token', '=', $token)->first();

		if($dt_token){
			$user = DB::table('tb_users')->where('id', '=', $dt_token->userId)->first();
			return response()->json(['message' => 'valid', 'email' => $user->email, 'username' => $user->username],200);
		}else{
