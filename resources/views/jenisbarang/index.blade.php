...
<div class="page-titles">
  <h2> {{ $pageTitle }} <small> {{ $pageNote }} </small></h2>
	...
	<div id="jenisbarangView"></div>	
	<div id="jenisbarangGrid">
		<div class="card">
			<div class="card-body">
			...
			<div class="table-responsive">			
				<table id="jenisbarangTable" class="display table  table-striped table-hover" cellspacing="0" width="100%">
			        <thead>
			            <tr>
			            	<th>ID</th>	
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#jenisbarangTable tbody').on('click', 'td.details-control', function () {
	        ...
	        if ( row.child.isShown() ) {
	            ...
	            tr.addClass('shown');
	            $.get('{{ url("jenisbarang/")}}/'+id , function(callback){
	            	$('#'+id).html(callback);
	            	$('#'+id).addClass('data');
	            })
	    ...
	    $('#jenisbarangTable').Sdtable({
	    	tableId : '#jenisbarang',
	    	table   : table,
	    	action  : '{{ url("jenisbarang")}}' 
	    });
